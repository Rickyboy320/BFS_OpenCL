...
//----------------------------------------------------------
void run_bfs_cpu(int no_of_nodes, Node *h_nodes, int edge_list_size, int *h_edges, char* h_mask, char* h_updating_graph_mask, int* h_visited, int *h_cost_ref)
{
#ifdef PROFILING
...
//----------------------------------------------------------
void run_bfs_opencl(int no_of_nodes, Node *h_nodes, int* h_frontier, int h_frontier_size, int edge_list_size, int *h_edges, int *h_visited, int *h_cost)
{
    int amtloops = 0;
    int h_new_frontier_size = 0;
    int* h_new_frontier = (int*) malloc(sizeof(int) * no_of_nodes);
    ...
    try
    {
        ...
        d_graph_nodes = _clMallocRW(no_of_nodes * sizeof(Node));
        d_graph_frontier = _clMallocRW(no_of_nodes * sizeof(int));
        d_graph_frontier_size = _clMallocRW(sizeof(int));
        d_graph_edges = _clMallocRW(edge_list_size * sizeof(int));
        d_new_frontier = _clMallocRW(no_of_nodes * sizeof(int));
        d_new_frontier_size = _clMallocRW(sizeof(int));
        d_graph_visited = _clMallocRW(no_of_nodes * sizeof(int));
        
        d_cost = _clMallocRW(no_of_nodes * sizeof(int));

        cl_event h2dpreevents[6];
        h2dpreevents[0] = _clMemcpyH2D(d_graph_nodes, no_of_nodes * sizeof(Node), h_nodes);
        h2dpreevents[1] = _clMemcpyH2D(d_graph_frontier, no_of_nodes * sizeof(int), h_frontier);
        h2dpreevents[2] = _clMemcpyH2D(d_graph_frontier_size, sizeof(int), &h_frontier_size);
        h2dpreevents[3] = _clMemcpyH2D(d_graph_edges, edge_list_size * sizeof(int), h_edges);
        h2dpreevents[4] = _clMemcpyH2D(d_graph_visited, no_of_nodes * sizeof(int), h_visited);
        ...
        do
        {
            ...
            _clSetArgs(kernel_id, kernel_idx++, d_graph_frontier);
            _clSetArgs(kernel_id, kernel_idx++, d_graph_frontier_size);
            _clSetArgs(kernel_id, kernel_idx++, d_graph_edges);
            _clSetArgs(kernel_id, kernel_idx++, d_new_frontier);
            _clSetArgs(kernel_id, kernel_idx++, d_new_frontier_size);
            _clSetArgs(kernel_id, kernel_idx++, d_graph_visited);
            _clSetArgs(kernel_id, kernel_idx++, d_cost);
            _clSetArgs(kernel_id, kernel_idx++, &no_of_nodes, sizeof(int));

            //int work_items = no_of_nodes;
            kernelstrings[0] = "Top-Down cycle w/ size: " + std::to_string(h_frontier_size);
            kernelevents[0] = _clInvokeKernel(kernel_id, h_frontier_size, work_group_size);
#ifdef PROFILING
            waitAndTime(1, kernelevents, kernelstrings, &kernel_timer);
#endif
            clReleaseEvent(kernelevents[0]);

            d2hevents[0] = _clMemcpyD2H(d_new_frontier_size, sizeof(int), &h_frontier_size);
#ifdef PROFILING
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        h_edges = (int*) malloc(sizeof(int) * edge_list_size);
        ...
        h_frontier = (int *)malloc(sizeof(int) * no_of_nodes);
        h_updating_graph_mask = (char *)malloc(sizeof(char) * no_of_nodes);
        h_visited = (int *)malloc(sizeof(int) * no_of_nodes);
        ...
        {    
            ...
            h_frontier[0] = source;
            h_visited[source] = 1;
            run_bfs_opencl(no_of_nodes, h_nodes, h_frontier, 1, edge_list_size, h_edges, h_visited, h_cost[i]);
        }
        ...
        run_bfs_cpu(no_of_nodes, h_nodes, edge_list_size, h_edges, h_mask, h_updating_graph_mask, h_visited, h_cost_ref);
        //---------------------------------------------------------
        //--result verification
        for(int i = 0; i < iterations; i++) {
